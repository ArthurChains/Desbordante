...
using std::deque, std::vector, std::cout, std::endl, std::filesystem::current_path;
...
TEST(pliChecker, first){
    ...
    auto path = current_path().append("inputData").append("Test1.csv");
...
TEST(pliIntersectChecker, first){
    ...
    std::unique_ptr<PositionListIndex> intersection;

    try {
        auto path = current_path().append("inputData");
